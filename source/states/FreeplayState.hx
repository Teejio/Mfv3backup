...
class FreeplayState extends MusicBeatState
{
	...
	override function create()
	{
		...
		trans.setGraphicSize(FlxG.width, FlxG.height);
		add(trans);

		swirl = new FlxSprite(0,0);
	...
	override function update(elapsed:Float)
	{
		...
		else if (controls.ACCEPT)
		{
			...
			new FlxTimer().start(wait, function(tmr:FlxTimer)
				{
				FlxG.camera.fade(FlxColor.BLACK, 0.2, false, null);
				}
			);
			new FlxTimer().start(wait + 0.2, function(tmr:FlxTimer)
				{
	...
	function changeSelection(change:Int = 0, playSound:Bool = true)
	{
		...
		if (iconArray[curSelected].graphic.width > 400)
