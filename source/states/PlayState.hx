...
class PlayState extends MusicBeatState
{
	...
	public static var ratingStuff:Array<Dynamic> = [
		['HOW ABOUT PLAY TUTORIAL WEAKLING?!', 0.2], //From 0% to 19%
		['Pretty Bad!!!', 0.4], //From 20% to 39%
		['Unqualified', 0.5], //From 40% to 49%
		['Bronze...', 0.6], //From 50% to 59%
		['Silver', 0.69], //From 60% to 68%
		['Gold', 0.7], //69%
		['Platinum', 0.8], //From 70% to 79%
		['Diamond', 0.9], //From 80% to 89%
		['Supreme', 1], //From 90% to 99%
	...
	public var blackCover:FlxSprite; //BLACK CLOVER AAAAAAAAAAAAAAAAAAAAAAAAA
	public var cloneCover:FlxSprite;
	...
	override public function create()
	{
		...
		blackCover = new FlxSprite();
		blackCover.makeGraphic(480, 790, 0xFF000000);
		blackCover.alpha = (ClientPrefs.data.focusOpacity / 100);
		blackCover.visible = ClientPrefs.data.increaseFocus;

		blackCover.screenCenter(XY);

		if(!ClientPrefs.data.middleScroll) blackCover.x += 315;
		add(blackCover);

		

		cloneCover = new FlxSprite();
		cloneCover.makeGraphic(480, 790, 0xFF000000);
		cloneCover.alpha = (ClientPrefs.data.focusOpacity / 100);
		cloneCover.visible = ClientPrefs.data.increaseFocus;

		if(!ClientPrefs.data.middleScroll) cloneCover.x += 80;	

		
		add(cloneCover);

		if(ClientPrefs.data.middleScroll) cloneCover.visible = false;
		...
		timeTxt = new FlxText(STRUM_X + (FlxG.width / 2) - 248, 19, 770, "", 35);
		timeTxt.screenCenter(X);
		timeTxt.setFormat(Paths.font("apple_kid.ttf"), 35, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		var sanesCheck:String = Std.string(SONG.song).toLowerCase();

		if(sanesCheck == 'eat will ya' || sanesCheck == 'eat-will-ya')
			timeTxt.setFormat(Paths.font("comic.ttf"), 26, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);

		timeBar = new HealthBar(0, timeTxt.y + (timeTxt.height / 4), 'timeBar', function() return songPercent, 0, 1);
		timeBar.y -= 10; //IT TRIGGERS FOR THE TEXT OF MOTHER ITSELF
		...
		comboOver = new FlxSprite();
		comboOver.loadGraphic(Paths.image('comboOver'));				
		comboOver.screenCenter(XY);
		comboOver.scrollFactor.set();	
		

		//By Kayne's Request
		if((ClientPrefs.data.ratingPosition == 'Right'))			
			comboOver.x += 515;
		else
			comboOver.x -= 515;
		...
		comboOver.antialiasing = true;

		add(comboOver);

		comboTxt = new FlxText(0, 0, Std.int(comboOver.width), "");
		comboTxt.setFormat(Paths.font("apple_kid.ttf"), 50, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		comboTxt.scrollFactor.set();
		
		comboTxt.screenCenter();
		comboTxt.setPosition(comboOver.x, comboOver.y + 5); //since this font has a little offset
		...
		scoreTxt = new FlxText(0, healthBar.y + 38, FlxG.width, "", 25);
		scoreTxt.setFormat(Paths.font("apple_kid.ttf"), 25, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		botplayTxt.setFormat(Paths.font("apple_kid.ttf"), 32, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		cloneCover.cameras = [camHUD];
		comboOver.cameras = [camHUD];
	...
	public function updateScore(miss:Bool = false)
	{
		...
		scoreTxt.text = 'Total Score: ' + songScore
	...
	function startSong():Void
	{
		...
		FlxTween.tween(blackCover, {alpha: (ClientPrefs.data.focusOpacity / 100)}, 0.5, {ease: FlxEase.circOut});
		FlxTween.tween(cloneCover, {alpha: (ClientPrefs.data.focusOpacity / 100)}, 0.5, {ease: FlxEase.circOut});
	...
	override public function onFocusLost():Void
	{
		...
		#end
		if (!paused && ClientPrefs.data.autoPause) {
			 doPause();
		}
		super.onFocusLost();
	}
	...
	override public function update(elapsed:Float)
	{
		...
		iconP1.animation.curAnim.curFrame = (healthBar.percent < 25) ? 1 : (healthBar.percent > 75 && iconP1.graphic.width > 400) ? 2 : 0;
		iconP2.animation.curAnim.curFrame = (healthBar.percent > 75) ? 1 : (healthBar.percent < 25 && iconP2.graphic.width > 400) ? 2 : 0;


		comboTxt.text = 'x' + Std.string(combo);
		...
		if(boyfriend.animation.curAnim.name.startsWith('sing'))
			{
				if(confirm)					
					scoreTxt.setFormat(Paths.font("comic.ttf"), 18, FlxColor.fromRGB(boyfriend.healthColorArray[0], boyfriend.healthColorArray[1], boyfriend.healthColorArray[2]), CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
				else
					scoreTxt.setFormat(Paths.font("apple_kid.ttf"), 25, FlxColor.fromRGB(boyfriend.healthColorArray[0], boyfriend.healthColorArray[1], boyfriend.healthColorArray[2]), CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);		
			}
		else
			{
				if(confirm)					
		...
		//Fade out the rating instead of using the og code 'cus for some reason they dont work on the function
		if(comboOver.alpha > 0 && comboTxt.alpha > 0)
			{					
				comboOver.alpha -= 0.025;
				comboTxt.alpha -= 0.025;					
			}		
		...
		else if (!paused && updateTime)
		{
			...
			var daSong:String = songName.toLowerCase();

			if(ClientPrefs.data.timeBarType != 'Song Name')
				{
					if(daSong == 'eat will ya' || daSong == 'eat-will-ya') 
						timeTxt.text = 'Eat Something Will Ya?' + ': ' + FlxStringUtil.formatTime(secondsTotal, false);
					else timeTxt.text = SONG.song + ': ' + FlxStringUtil.formatTime(secondsTotal, false);
				}
	...
	public function endSong()
	{
		...
		if(ret != FunkinLua.Function_Stop && !transitioning)
		{
			...
			if (chartingMode)
			{
				...
				CartridgeHandler.removeNoti(SONG.song);
			if (isStoryMode)
			{
				...
				if (storyPlaylist.length <= 0)
				{
					...
					if(!ClientPrefs.getGameplaySetting('practice') && !ClientPrefs.getGameplaySetting('botplay')) {
						...
						trace("finished Week");
						Cartridge.unlockWeek(WeekData.getCurrentWeek().weekName);
	...
	private function popUpScore(note:Note = null):Void
	{
		...
		rating.acceleration.y = 1 * playbackRate * playbackRate;
		rating.velocity.y -= 1 * playbackRate;
		rating.velocity.x -= 1 * playbackRate;
		...
		//insert(members.indexOf(strumLineNotes), comboOver);
		//insert(members.indexOf(strumLineNotes), comboTxt);
		...
		if (!PlayState.isPixelStage)
		{
			...
			comboSpr.setGraphicSize(Std.int(comboSpr.width * 1));
		}
		else
		{
	...
	function noteMissPress(direction:Int = 1):Void //You pressed a key when there was no notes to press for this key
	{
		...
		FlxG.sound.play(Paths.soundRandom('missnote', 1, 3), FlxG.random.float(0.5, 0.8));
	...
	{
		//FlxG.sound.play(Paths.soundRandom('missnote', 1, 3), FlxG.random.float(0.5, 0.8));
		FlxFlicker.flicker(scoreTxt, 0.45, 0.03, true); 

		var daSong:String = songName.toLowerCase();
		var gameChosen:Int = 0;

		switch(daSong)
		{
			case 'encore' | 'poindexter' | 'angeles' | 'dissonance' :
				//trace("It's one song from Mother 1");
				gameChosen = 1;

			case 'rockin' | 'dakota' | 'checksum' | 'rage' | 'scarlet' | 'fire at will' | 'fire-at-will' | 'eat will ya' | 'eat-will-ya':
				//trace("It's one song from Mother 2 // Earthbound");
				gameChosen = 2;

			case 'love' | 'rtc' | 'mother3real' | 'warforged':
				//trace("It's one song from Mother 3");
				gameChosen = 3;
		...
		FlxG.sound.play(Paths.sound('miss' + Std.string(gameChosen)), FlxG.random.float(0.7, 0.875));
	...
	//HAD TO CREATE A OWN SINCE POP-UP RATINGS DOESNT WORK PROPERLY FROM THE OG
	public function displayCombo()
		{
			comboTxt.alpha = 1;
			comboOver.alpha = 1;
		}
