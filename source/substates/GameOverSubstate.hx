...
class GameOverSubstate extends MusicBeatSubstate
{
	...
	var songList:Array<String> = [ 'Rockin', "Encore", "Dissonance", "Angeles"];
	var picoList:Array<String> =  [ "Angeles" ];
	...
	public function new(x:Float, y:Float, camX:Float, camY:Float)
	{
		...
		if (picoList.contains(PlayState.SONG.song)){
		...
		/**if (songList.contains(PlayState.SONG.song) == false){
			if (PlayState.SONG.song == "rtc"){
				FlxG.sound.play(Paths.sound('rtc'));
			}
			else
			{
				FlxG.sound.play(Paths.sound('die'), 0.5);
			}
