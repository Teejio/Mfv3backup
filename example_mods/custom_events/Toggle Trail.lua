...
function createTrailFrame(tag)
	...
	if tag == 'BF' then
		...
		if trailEnabledBF then
			color = getColorFromHex(getHealthColor('boyfriend'))
		...
		if trailEnabledDad then
			color = getColorFromHex(getHealthColor('dad'))
	...
	if not (image == '') then
		...
		setProperty(trailTag..'.alpha', 0.7);
	...
	if num - trailLength + 1 >= 0 then
		for i = (num - trailLength + 1), (num - 1) do
			...
			setBlendMode('psychicTrail'..tag..i, 'screen')

			if tag == 'Dad' then
			--setBlendMode('psychicTrail'..tag..i, 'shader')
			end
		end
	end
